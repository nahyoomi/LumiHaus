...
const Button: React.FC<ButtonProps> = ({ text, onClick, variant = 'tonal', className = '', type='button'}) => {
  ...
  return (
    <button onClick={onClick} type={type} className={`${baseClasses} ${variantClasses} ${className}`}>
      {text}
    </button>
  );
};
